...
async function syncToSheetValidate({ phone, link }) {
  ...
  const api = `${apis.jp24.urlSyncGoogleSheetSpam}?phone=${phone}&link=${link}&SHEET_NAME=SDTValidate`;
  await fetch(
    `${apis.jp24.urlSyncGoogleSheetSpam}?phone=${phone}&link=${link}&SHEET_NAME=SDTValidate`,
    {
      method: "GET",
      mode: "no-cors",
...
async function syncToSheetServerFail({
  ...
  link = link.indexOf("&") > -1 ? link.replaceAll("&", "_SKYCOM_") : link;
  await fetch(
    `${apis.jp24.urlSyncGoogleSheetSpam}?time=${timeFirstRenderPage.toLocaleDateString()}-${timeFirstRenderPage.toLocaleTimeString()}&name=${name}&phone=${phone}&utmSource=${
      ad_channel || ""
    }&utmMedium=${
      ad_account || ""
...
async function handlePostData({
  ...
  const response = await fetch(apis.jp24.bareURL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    ...
    .then(async function (data) {
      ...
      if (data.spam) {
        window.parent.location.replace(`${apis.jp24.urlThankFake}?name=${Ten1}&phone=${Ten2}`);
      } 
      else if(data.suspect) {
        window.parent.location.replace(`${apis.jp24.urlConfirm}?lead_id=${data.lead_id}`); 
      }
      else{
        window.parent.location.replace(`${apis.jp24.urlThankReal}?name=${Ten1}&phone=${Ten2}`);
      }
    })
