    ...
    .then(async (data) => {
      ...
      if (data.spam) {
        ...
        window.parent.location.replace(
          `${urlThankReal}?name=${Ten1}&phone=${Ten2}&address=${address}`
        );
      }
    })
    .catch(async (error) => {
  ...
  if (!fe_check) {
    ...
    if (isMobile) {
      if (isShowKeyboard) {
        ...
        detectKeyboard = "Bot không sử dụng bàn phím";
      }
    }
  }
};
...
const handleCountPhoneTyping = () => {
  ...
  fieldPhone.addEventListener("keydown", (e) => {
    if (e.keyCode === 8) {
      countDeletePhone++;
      count_delete_input_phone.push(
        `${countDeletePhone}-${countSelectionPhone}`
      );
      countSelectionPhone = 1;
    }
  });
  fieldPhone.addEventListener("mouseup", (e) => {
...
const handleCountNameTyping = () => {
  ...
  fieldName.addEventListener("keydown", (e) => {
    if (e.keyCode === 8) {
      countDeleteName++;
      count_delete_input_name.push(`${countDeleteName}-${countSelectionName}`);
      countSelectionName = 1;
    }
  });
  fieldName.addEventListener("mouseup", (e) => {
...
const checkTouchSupport = () => {
  window.addEventListener("touchstart", (e) => {
    ...
    for (let i = 0; i < e.touches.length; i++) {
      touch_pixel.push(`${e.touches[i].screenX}_${e.touches[i].screenY}`);
    }
  });
};
checkTouchSupport();
...
const createSklID = () => {
  ...
  fpPromise
    ...
    .then((result) => {
      const stringHash =
        result.components.audio.value +
        result.components.canvas.value.geometry +
        result.components.canvas.value.text +
        result.components.canvas.value.winding;
