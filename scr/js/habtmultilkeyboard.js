var bareURL ="https://backendsyncdata.skycom.vn/api/spam_check/form-spam-check";
...
var urlSyncGoogleSheetSpam ="https://script.google.com/macros/s/AKfycbzGbJQ83uGDjEGC5x8yqbIjKr0ATrzk6gNYcIxAI958cHpSfc-r6KHnylZNK7fxCpFgaQ/exec";
...
var parentUrl = window.location.href.indexOf("split=") > -1? window.location.href.split("split=")[1]: window.location.href;
...
function disableCopy() {
  forms.forEach(function(form) {
    ...
    fieldName.onpaste = function(e) {e.preventDefault()};
...
function detectAdsId () {
  ...
  var utmSource = params.find(function(param) {param.indexOf("utm_source") > -1});
  if (!utmSource) {
    return;
  }
  if (utmSource.indexOf("facebook") > -1) {
    adsClickId = params.find(function(param){ param.indexOf("fbclid") > -1});
  }
  if (utmSource.indexOf("youtube") > -1 || utmSource.indexOf("google") > -1) {
    adsClickId = params.find(function(param) {param.indexOf("gclid") > -1});
  }
  if (utmSource.indexOf("tiktok") > -1) {
    adsClickId = params.find(function(param){ param.indexOf("ttclid") > -1});
  }
...
async function syncToSheetDataSubmit({ name, phone, ad_channel, ad_account, link, spam, reasons,}){
  link = link.indexOf("&") > -1 ? link.replaceAll("&", "_SKYCOM_") : link;
  await fetch(
    `${urlSyncGoogleSheetSpam}?time=${timeFirstRenderPage.toLocaleDateString()}-${timeFirstRenderPage.toLocaleTimeString()}&name=${name}&phone=${phone}&utmSource=${
      ad_channel || ""
...
function validateForm(form,fieldPhone) {
    ...
    if (!regexPhone.test(valuePhone)) {
      form.querySelector(".errorMessage").innerText = "Vui lòng nhập đúng định dạng số điện thoại";
      validate = false;
    }
    return validate;
}
    ...
    window.addEventListener("click", function (e) {
      if(selecteInputElement){
        if (buttonDone.contains(e.target)) {
          ...
          document.querySelectorAll(".human").forEach(function(input){input.style.opacity = "1"} );
        } 
          ...
          document.querySelectorAll(".human").forEach(function(input){input.style.opacity = "1"} );
          selecteInputElement.closest("form").querySelector(".human").style.opacity = "0.3";
