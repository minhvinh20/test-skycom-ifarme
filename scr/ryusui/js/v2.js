...
// ============================== Lấy thông tin từ radioGroup =================================

function getFormOptionValue(name, variable) {
  let radioGroup = document.getElementsByName(name);
  radioGroup.forEach((item) => {
    if (item.checked) variable = item.value;
  });
  return variable;
}

function handleHairtype() {
  for (var i = 1; i < 4; i++) {
    let value = document.querySelector(`input[name="hair_${i}"]:checked`)
      ? document.querySelector(`input[name="hair_${i}"]:checked`).value
      : "";
    if (value) {
      Hair_type += `${value}, `;
    }
