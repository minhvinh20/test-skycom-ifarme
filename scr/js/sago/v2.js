...
function listenerKeyboardOpen(){
  ...
  let isKeyboardOpen = isMobile && window.screen.height - MIN_KEYBOARD_HEIGHT > window.visualViewport.height;
...
const fieldAddress = document.getElementById(encodeAddress);
...
function disableCopy() {
    ...
    fieldAddress.onpaste = function(e) {e.preventDefault()};
}
disableCopy();
...
function setTimer() {
  ...
  fieldAddress?.addEventListener("focus", () => {
    if (!timeAdIn) timeAdIn = new Date();
  });
  fieldAddress?.addEventListener("focusout",function(e) {
    timeAdOut = new Date();
  });
}
setTimer();
...
function inputTiming() {
  ...
  var Action_ad_time      = Math.abs(timeAdOut - timeAdIn) / 1000;
...
async function syncToSheetServerFail ({ name, phone, address, ad_channel, ad_account, link, reasons,}) {
  link = link.indexOf("&") > -1 ? link.replaceAll("&", "_SKYCOM_") : link;
  await fetch(
    `${urlSyncGoogleSheetSpam}?time=${timeFirstRenderPage.toLocaleDateString()}-${timeFirstRenderPage.toLocaleTimeString()}&name=${name}&phone=${phone}&address=${address}&utmSource=${
      ad_channel || ""
    }&utmMedium=${
      ad_account || ""
...
async function syncToSheetDataSubmit({ name, phone, address, ad_channel, ad_account, link, spam, reasons,}){
  link = link.indexOf("&") > -1 ? link.replaceAll("&", "_SKYCOM_") : link;
  await fetch(
    `${urlSyncGoogleSheetSpam}?time=${timeFirstRenderPage.toLocaleDateString()}-${timeFirstRenderPage.toLocaleTimeString()}&name=${name}&phone=${phone}&address=${address}&utmSource=${
      ad_channel || ""
